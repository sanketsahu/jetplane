...
// @ts-ignore - sibling ESM helper
import { generateCssModuleSource, cssCacheModuleRel, isNativeWindProject, startTailwindWatch, nativeCssModuleSource, cssInputPath } from './jetplane-css.mjs'
...
async function freshenAtBoot() {
  ...
  for (const p of plats) {
    ...
    try {
      ...
      stashRouteEntries(p, [...upd.modified, ...upd.added])
...
// className styling flows through ONE compiled registry module in each bundle,
// generated by tailwind from the source tree AT BAKE TIME. Files added after the
// bake use classes the registry has never seen — their modules hot-load but render
// UNSTYLED. A PERSISTENT tailwind watch child (nativewind's own) emits fresh CSS
// ~1-2s after any content change; each emission is pushed as a hot update to the
// registry module AND the recently pushed route modules are re-sent, so mounted
// components re-resolve their classNames without a manual reload.
const isNW = isNativeWindProject(projectDir)
const lastCssHash = new Map<string, string>()
// route-module entries pushed since the last registry update, per platform — these
// get RE-pushed after the registry lands (a mounted component that rendered before
// its classes existed never re-resolves on its own).
...
function sendUpdate(platform: string, body: { added: any[]; modified: any[] }) {
  ...
  const msg = JSON.stringify({ type: 'update', body: { revisionId: String(rev), added: body.added, modified: body.modified, deleted: [] } })
...
// one-shot fallback, used by the periodic reconciler in case the watch child's own
// file watching goes deaf (same gVisor caveat as ours)
let cssBusy = false
async function refreshCss(reason: string) {
  if (!isNW || cssBusy) return
  cssBusy = true
  try {
    const { source } = await generateCssModuleSource(projectDir, 'ios')
    for (const p of targets.keys()) {
      const t = targets.get(p)!
      const rel = cssCacheModuleRel(t.maps, p)
      if (!rel) continue
      const h = crypto.createHash('sha256').update(source).digest('hex')
      if (lastCssHash.get(`oneshot:${p}`) === h) continue
      lastCssHash.set(`oneshot:${p}`, h)
    ...
    console.log('css: one-shot refresh skipped -', e.message)
  } finally { cssBusy = false }
}
...
function startCssWatchers() {
  ...
  spawnWatch('ios', applyNativeCss)
  if (hasWeb) spawnWatch('web', applyWebCss)
}
...
const onSourceEvent = (base: string) => (_e: any, file: any) => {
  if (!file) return
  // css edits are the tailwind watch child's territory — it re-emits on its own
  if (/\.css$/.test(String(file))) return
...
function reconcileTick() {
  if (updating) return
  // every ~5 min, a one-shot css check backstops the tailwind watch child (its own
  // file watching has the same gVisor caveat as ours)
  if (++reconcileN % 15 === 0) refreshCss('reconciler')
...
// Start the persistent tailwind watchers without blocking the port (wakes must stay
// instant). The first emission arrives seconds later and hot-applies; content changes
// re-emit within ~1-2s, and the recently pushed screens are re-sent afterwards so
// they pick up their styles without a reload.
startCssWatchers()
